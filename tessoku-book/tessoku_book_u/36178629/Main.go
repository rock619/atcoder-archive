...
func solve(o Printer, N int, P []int, A []int) {
	dp := Make2D(N, N, 0)
	for i := 0; i < N; i++ {
		for j := N - 1; j >= i; j-- {
			if i > 0 {
				UpdateMax(&dp[i][j], dp[i-1][j])
				if x := P[i-1] - 1; x >= i && x <= j {
					UpdateMax(&dp[i][j], dp[i-1][j]+A[i-1])
				}
			}
			if j < N-1 {
				UpdateMax(&dp[i][j], dp[i][j+1])
				if x := P[j+1] - 1; x >= i && x <= j {
					UpdateMax(&dp[i][j], dp[i][j+1]+A[j+1])
				}
			}
		}
	}
