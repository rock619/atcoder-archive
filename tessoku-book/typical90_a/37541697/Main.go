...
func solve(o Printer, N int, L int, K int, A []int) {
	a := make([]int, N+1)
	for i := range A {
		if i == 0 {
			a[i] = A[i]
	...
	v := sort.Search(1000_000_000, func(i int) bool {
		return !Satisfiable(a, K, i)
	})
	v--
	o.l(v)
}

func Satisfiable(a []int, K, length int) bool {
	count := 0
	for current, i := 0, 0; i < len(a); i++ {
		current += a[i]
		if current < length {
			continue
		}
