...
func solve(o Printer, N int, Q int, S []byte, a []int, b []int, c []int, d []int) {
	rollingHash := NewRollingHash(N)
	hash := rollingHash.Of(string(S))
	for i := 0; i < Q; i++ {
		if rollingHash.Slice(hash, a[i]-1, b[i]) == rollingHash.Slice(hash, c[i]-1, d[i]) {
			o.l(yes)
		} else {
			o.l(no)
		}
