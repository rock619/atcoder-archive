...
func solve(o Printer, N int, T []int, D []int) {
	exams := make([]Exam, N)
	for i := range T {
		exams[i] = Exam{T: T[i], D: D[i]}
	}
	sort.Slice(exams, func(i, j int) bool {
		return exams[i].D < exams[j].D
	})

	dp := Make2D(N+1, 1440+1, -1<<62)
	dp[0][0] = 0
	for i := 1; i <= N; i++ {
		for j := 0; j <= 1440; j++ {
			dp[i][j] = dp[i-1][j]
			if j <= exams[i-1].D && j-exams[i-1].T >= 0 {
				UpdateMax(&dp[i][j], dp[i-1][j-exams[i-1].T]+1)
			}
		}
	}
	o.l(Max(dp[N]...))
