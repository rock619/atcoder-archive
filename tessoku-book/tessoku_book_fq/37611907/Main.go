...
func solve(o Printer, N int, L int, K int, A []int, C []int) {
	minPrices := make([]int, L+1)
	for i := range minPrices {
		minPrices[i] = inf
	}
	for i := range A {
		UpdateMin(&minPrices[A[i]], C[i])
	}

	st := NewSegTree(minPrices)
	result := 0
	for i := 1; i <= L-K; i++ {
		if min := st.Prod(i, i+K); min == inf {
			o.l(-1)
			return
		} else {
			result += min
