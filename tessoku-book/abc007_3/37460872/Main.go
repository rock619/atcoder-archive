...
func solve(o Printer, R, C, sx, sy, gx, gy int, c [][]byte) {
	q := NewQueue(R * C)
	sID := ID(C, sx, sy)
	gID := ID(C, gx, gy)
	q.Enqueue(sID)
	...
	for !q.Empty() {
		...
		for _, next := range Nexts(R, C, id) {
			x, y := XY(C, next)
			if c[y-1][x-1] == '#' || next == sID || result[next] > 0 {
				continue
			}
...
func ID(C, x, y int) int {
	return (y-1)*C + x - 1
}

func XY(C, id int) (x, y int) {
	return id%C + 1, id/C + 1
}

func Nexts(R, C, id int) []int {
	x, y := XY(C, id)
	result := []int{}
	for _, next := range []int{ID(C, x-1, y), ID(C, x+1, y), ID(C, x, y-1), ID(C, x, y+1)} {
		if next >= 0 && next < R*C {
