...
func solve(o Printer, N int, h []int) {
	dp := make([]int, N)
	for i := 1; i < N; i++ {
		if i == 1 {
			dp[i] = Abs(h[i] - h[i-1])
	...
	result := make([]int, 1, N)
	result[0] = N
	for current := N; current != 1; current = result[len(result)-1] {
		if current == 2 || dp[current-1] == dp[current-1-1]+Abs(h[current-1]-h[current-1-1]) {
			result = append(result, current-1)
		} else {
			result = append(result, current-2)
		}
	}
	Reverse(result)
	o.l(len(result))
	o.l(strings.Trim(fmt.Sprint(result), "[]"))
}
