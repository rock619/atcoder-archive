...
func solve(o, lg Printer, N int, A []int, Q int, queries []Query) {
	indexes := Make2D(N+1, 0, 0)
	for i, v := range A {
	...
	for _, q := range queries {
		x := q.X
		l := sort.Search(len(indexes[x]), func(i int) bool {
			return q.L-1 <= indexes[x][i]
		})
		if l >= len(indexes[x]) {
			o.l(0)
			continue
		}
		r := sort.Search(len(indexes[x]), func(i int) bool {
			return q.R-1 < indexes[x][i]
		})
		r--
		o.l(r - l + 1)
	}
...
func main() {
	...
	logger := NewLogger()
	solve(out, logger, N, A, Q, queries)
