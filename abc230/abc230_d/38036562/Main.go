...
func solve(o, lg Printer, N int, D int, L []int, R []int) {
	walls := make([]Wall, N)
	for i := range walls {
		walls[i] = Wall{L: L[i], R: R[i]}
	}
	sort.Slice(walls, func(i, j int) bool {
		if walls[i].R == walls[j].R {
			return walls[i].L < walls[j].L
		}
		return walls[i].R < walls[j].R
	})
	lg.f("%#v", walls)
	count := 0
	for i := 0; i < N; {
