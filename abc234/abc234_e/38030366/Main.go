...
func Calc(digits, begin, diff int) (int, bool) {
	if diff == 0 {
		i, _ := strconv.Atoi(strings.Repeat(strconv.Itoa(begin), digits))
		return i, true
	}
	result := begin
	for i := 1; i < digits; i++ {
		digit := result%10 + diff
		if digit < 0 || digit > 9 {
			return 0, false
		}
...
func solve(o, lg Printer, X int) {
	digits := Digits(X)
	for begin := digits[len(digits)-1]; begin <= 9; begin++ {
		for diff := -9; diff <= 9; diff++ {
			v, ok := Calc(len(digits), begin, diff)
			lg.p(begin, diff, v, ok)
			if !ok || v < X {
				continue
			}
			o.l(v)
