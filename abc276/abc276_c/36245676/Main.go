...
func solve(o Printer, N int, P []int) {
	// log.Print(P)
	result := make([]int, N)
	copy(result, P)
	min := 1 << 62
	...
	result[targetIndex], result[nextIndex] = result[nextIndex], result[targetIndex]
	sort.Ints(result[targetIndex+1:])
	Reverse(result[targetIndex+1:])
