...
func solve(o Printer, N int, M int, A []int, B []int) {
	roads := make([]Road, 0, 2*M)
	for i := range A {
		a, b := A[i], B[i]
		roads = append(roads, Road{A: a, B: b})
		roads = append(roads, Road{A: b, B: a})
	}
	sort.Slice(roads, func(i, j int) bool {
		if roads[i].A == roads[j].A {
			return roads[i].B < roads[j].B
		}
		return roads[i].A < roads[j].A
	})

	result := make([][]int, N+1)
	for _, r := range roads {
		result[r.A] = append(result[r.A], r.B)
	}
