...
func solve(o, lg Printer, N int, K int, D int, a []int) {
	dp := make([][][]int, N+1)
	for i := range dp {
		dp[i] = Make2D(N+1, D, -inf)
	}
	dp[0][0][0] = 0
	for i := 1; i <= N; i++ {
		for j := 1; j <= i; j++ {
			for k := 0; k < D; k++ {
				...
				UpdateMax(&dp[i][j-1][k], dp[i-1][j-1][k])
				v := dp[i-1][j-1][k] + a[i-1]
				UpdateMax(&dp[i][j][v%D], v)
			}
		}
	}
	lg.f("%+v", dp)
	if result := dp[N][K][0]; result < 0 {
		o.l(-1)
	} else {
		o.l(result)
	}
