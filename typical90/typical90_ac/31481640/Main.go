...
func solve(W int64, N int64, L []int64, R []int64) {
	...
	compressed := make([]int64, 0, 2*N)
	for i := int64(0); i < N; i++ {
		compressed = append(compressed, L[i], R[i])
	}
	sort.Slice(compressed, func(i, j int) bool {
		return compressed[i] < compressed[j]
	})
	compressed = dedup(compressed)
	for i := int64(0); i < N; i++ {
		L[i] = int64(sort.Search(len(compressed), func(j int) bool {
			return compressed[j] >= L[i]
		}))
		R[i] = int64(sort.Search(len(compressed), func(j int) bool {
			return compressed[j] >= R[i]
		}))
	}

	st := NewSegmentTree(int64(len(compressed)))

	for i := int64(0); i < N; i++ {
		h := st.RangeMax(L[i], R[i]+1) + 1
		st.Update(L[i], R[i]+1, h)
		fmt.Fprintln(w, h)
	}
}

func dedup(ints []int64) []int64 {
	result := ints[:1]
	for _, v := range ints {
