...
func solve(L, R int) {
	...
	fmt.Fprintln(w, (solve2(uint64(R))-solve2(uint64(L-1))+mod)%mod)
}

func solve2(R uint64) int {
	m := uint64(mod)
	inv2 := uint64(InvMod(2, mod))

	sum := uint64(0)
	for i := uint64(1); i <= R; i *= 10 {
		l := uint64(len(strconv.FormatUint(i, 10)))
		if 10*i > R {
			sum += (i + R) % m * ((R - i + 1) % m) % m * l % m * inv2 % m
			sum %= m
			break
		}
		sum += (i + 10*i - 1) % m * ((10*i - i) % m) % m * l % m * inv2 % m
		sum %= mod
	}
	return int(sum)
}
