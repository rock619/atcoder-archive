...
func solve(o Printer, H, W, rs, cs, N int, r, c []int, Q int, d []byte, l []int) {
	...
	wallsByRowRev := make(map[int][]int)
	for k := range wallsByRow {
		sort.Ints(wallsByRow[k])
		wallsByRowRev[k] = Reversed(wallsByRow[k])
	}
	wallsByColRev := make(map[int][]int)
	for k := range wallsByCol {
		sort.Ints(wallsByCol[k])
		wallsByColRev[k] = Reversed(wallsByCol[k])
	}
	...
	for i := 0; i < Q; i++ {
		...
		case 'L':
			...
			index := sort.Search(len(wallsByRowRev[row]), func(j int) bool {
				return wallsByRowRev[row][j] < col
			})
			if index >= len(wallsByRowRev[row]) || wallsByRowRev[row][index] < min {
				col = min
				break
			}
			col = wallsByRowRev[row][index] + 1
		case 'R':
			max := col + li
			if max > W {
				max = W
		...
		case 'U':
			...
			index := sort.Search(len(wallsByColRev[col]), func(j int) bool {
				return wallsByColRev[col][j] < row
			})
			if index >= len(wallsByColRev[col]) || wallsByColRev[col][index] < min {
				row = min
				break
			}
			row = wallsByColRev[col][index] + 1
		case 'D':
			max := row + li
			if max > H {
				max = H
...
func Reversed(s []Element) []Element {
