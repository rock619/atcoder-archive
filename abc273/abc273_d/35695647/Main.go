	...
	for i := 0; i < Q; i++ {
		...
		case 'L':
			...
			index := sort.Search(len(wallsByRow[row]), func(j int) bool {
				jj := len(wallsByRow[row]) - 1 - j
				return wallsByRow[row][jj] < col
			})
			index2 := len(wallsByRow[row]) - 1 - index
			if index >= len(wallsByRow[row]) || wallsByRow[row][index2] < min {
				col = min
				break
			}
			col = wallsByRow[row][index2] + 1
		case 'R':
			max := col + li
			if max > W {
				max = W
		...
		case 'U':
			...
			index := sort.Search(len(wallsByCol[col]), func(j int) bool {
				jj := len(wallsByCol[col]) - 1 - j
				return wallsByCol[col][jj] < row
			})
			index2 := len(wallsByCol[col]) - 1 - index
			if index >= len(wallsByCol[col]) || wallsByCol[col][index2] < min {
				row = min
				break
			}
			row = wallsByCol[col][index2] + 1
		case 'D':
			max := row + li
			if max > H {
				max = H
